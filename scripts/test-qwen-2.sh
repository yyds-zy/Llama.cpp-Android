...
file_name=qwen_2_${gross_pruned_ratio}_${begin_pruned_layer}-${end_pruned_layer}

# 剪枝
echo ----------------------------- pruning --------------------------
cd ${work_dir}/LLM-Pruner
CUDA_VISIBLE_DEVICES=2 python qwen2.py \
...
cd ${work_dir}/convert
CUDA_VISIBLE_DEVICES=2 python convert_qwen2.py --qwen2_dir ${base_model} \
